...
public class PixelImage {
    ...
    public void computeNewImage(int [][] kernel, int divisor)
    {
        Pixel [][] result = this.getData();
            ...
            for (int col=1; col<this.getWidth()-1; ++col)
            {
                ...
                for(int i=-1; i<=1; ++i)
                {
                    for (int j =-1; j<=1; ++j)
                    {
                        ...
                        redSum += (p.red * kernel[i+1][j+1]);
                        greenSum += (p.green * kernel[i+1][j+1]);
                        blueSum += (p.blue * kernel[i+1][j+1]);
                    }
                }
